...
export default function DesignEmpresasSection() {
  ...
  useEffect(() => {
    ...
    // Abre projeto vindo da Home via query param
  useEffect(() => {
    const project = searchParams.get('project');

    if (!project) return;

    let targetProject: EmpresaProject | undefined;

    if (project === 'valbao-odontologia') {
      targetProject = empresasData.find((item) =>
        item.title.toUpperCase().includes('VALBÃO ODONTOLOGIA')
      );
    }

    if (project === 'eventos-estandes') {
      targetProject = empresasData.find((item) =>
        item.title.toUpperCase().includes('EVENTO E ESTANDE')
      );
    }

    if (targetProject?.gallery) {
